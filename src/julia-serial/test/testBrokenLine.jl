...
function testFit(N::Int)
    ...
    fast_fit_results = zeros(Float64, 4)
    Main.RecoPixelVertexing_PixelTrackFitting_interface_BrokenLine_h.BL_Fast_fit(hits, fast_fit_results)
    ...
    circle_fit_results = RecoPixelVertexing_PixelTrackFitting_interface_FitResult_h.circle_fit()
    ...
    line_fit_results = RecoPixelVertexing_PixelTrackFitting_interface_FitResult_h.line_fit()
    Main.RecoPixelVertexing_PixelTrackFitting_interface_BrokenLine_h.BL_Line_fit(hits_ge, fast_fit_results, B, data, line_fit_results)
    println("Fitted values (LineFit):")
    println("par: ", line_fit_results.par)
    println("chi2: ", line_fit_results.chi2)

    println("Fitted cov (LineFit): CPU")
    println(line_fit_results.cov)

    println("VALIDATED LINE FIT")
    println(" ")
    Main.RecoPixelVertexing_PixelTrackFitting_interface_BrokenLine_h.BL_Circle_fit(hits, hits_ge, fast_fit_results, B, data, circle_fit_results)
    ...
    Jacob[3, 1] = 0.0
    Jacob[3, 2] = 0.0
    Jacob[3, 3] = -B / copysign((circle_fit_results.par[3])^2, circle_fit_results.par[3])

    circle_fit_results.par[3] = B / abs(circle_fit_results.par[3])
    circle_fit_results.cov = Jacob * circle_fit_results.cov * Jacob'

    println("Fitted values (CircleFit):")
    println("Parameters: ", circle_fit_results.par)
    println("Chi-squared: ", circle_fit_results.chi2)

    println("Fitted covariance (CircleFit) CPU:")
    println(circle_fit_results.cov)

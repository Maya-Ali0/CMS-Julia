...
@inline function BL_Fast_fit(hits::Matrix{Float64}, results::Vector{Float64})
   n = size(hits,2)
   a = hits[1:2, div(n, 2) + 1] - hits[1:2, 1]
   b = hits[1:2, n] - hits[1:2, div(n, 2) + 1]
   c = hits[1:2, 1] - hits[1:2, n]

   tmp = 0.5 / cross2D(c,a)
   result[1] = hits[1,1] - (a[2] * squaredNorm(c) + c[2] * squaredNorm(a)) * tmp
   result[2] = hits[2,1] + (a[1] * squaredNorm(c) + c[1] * squaredNorm(a)) * tmp

   result[3] = sqrt( squaredNorm(a) * squaredNorm(b) * squaredNorm(c))
   
   d = hits[1:2, 1] - result[1:2]
   e = hits[1:2, n] - result[1:2]

   results[4] = result[3] * atan2(cross2D(d,e), d.*e)/ (hits[3:n-1] - hits[3,1])
end
...
@inline function BL_Circle_fit(hits::Matrix{Float64}, hits_ge::Matrix{Float64}, fast_fit::Vector{Float64}, B::Float64, data::PreparedBrokenLineData, circle_results::circle_fit)
    n = size(hits,2)
    ...
    circle_results.par[2] = - circle_results.q * (fast_fit[3] -sqrt(fast_fit[3])) - (norm(0.25 * (e - d)))^

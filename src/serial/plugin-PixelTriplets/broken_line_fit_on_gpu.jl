...
function launchBrokenLineKernelsOnCPU(fitter::HelixFitOnGPU, hv::HitsOnGPU, hitsInFit::UInt32, maxNumberOfTuples::UInt32)
    ...
    open("log.txt", "w") do log_file
        for offset in 1:maxNumberOfConcurrentFits:maxNumberOfTuples
            ...
            kernelBLFit(3, fitter.tuple_multiplicity_d, fitter.bField_, fitter.outputSoa_d, hitsGPU, hits_geGPU, fast_fit_resultsGPU, UInt32(3), UInt32(offset))
            flush(log_file)
            ...
            kernelBLFit(4, fitter.tuple_multiplicity_d, fitter.bField_, fitter.outputSoa_d, hitsGPU, hits_geGPU, fast_fit_resultsGPU, UInt32(4), UInt32(offset))
            flush(log_file)
            ...
            if fitter.fit5as4
                ...
                kernelBLFit(4, fitter.tuple_multiplicity_d, fitter.bField_, fitter.outputSoa_d, hitsGPU, hits_geGPU, fast_fit_resultsGPU, UInt32(5), UInt32(offset))
                flush(log_file)
            else
                ...
                kernelBLFit(5, fitter.tuple_multiplicity_d, fitter.bField_, fitter.outputSoa_d, hitsGPU, hits_geGPU, fast_fit_resultsGPU, UInt32(5), UInt32(offset))
                flush(log_file)
            end
        end
    end

...
function compute(self::CircleEq{T},x1::T,y1::T,x2::T,y2::T,x3::T,y3::T) where T
    no_flip::Bool = abs(x3-x1) < abs(y3-y1)
    x1p = noflip ? x1 - x2 : y1 - y2;
    y1p = noflip ? y1 - y2 : x1 - x2;
    d12 = x1p * x1p + y1p * y1p;
    x3p = noflip ? x3 - x2 : y3 - y2;
    y3p = noflip ? y3 - y2 : x3 - x2;
    d32 = x3p * x3p + y3p * y3p;
